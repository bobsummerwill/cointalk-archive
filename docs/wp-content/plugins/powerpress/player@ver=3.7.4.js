...
function powerpress_embed_quicktime(div,media_url,width,height,scale)
{
	if( document.getElementById(div) )
	{
		...
		if( contentType == 'video/mp4' || contentType == 'video/x-m4v' )
			Html += '<video autoplay="" controls="" height="'+ height +'" src="https://web.archive.org/web/*/http://cointalk.ca/wp-content/plugins/powerpress/'+ media_url +'" width="'+ width +'">';
		...
		Html += '	<embed autoplay="true" cache="true" controller="true" correction="full" height="'+ height +'" pluginspage="http://www.apple.com/quicktime/download/" scale="'+ scale +'" src="'+ media_url +'" type="'+ contentType +'" width="'+ width +'"/>';
		Html += '</object>\n';
		if( contentType == 'video/mp4' || contentType == 'video/x-m4v' )
			Html += '</video>';
...
function powerpress_embed_html5iframe(id, url, width, height)
{
	if( document.getElementById('powerpress_player_'+id) ) {
		var Html = '';
		Html += '<iframe'; &&="" )="" *="" **="" +=' class="powerpress-player-embed"' +'"';="" -="" ;="" @div="" @height="" @media_url="" @webm_media_url="" @width="" alternative="" content="" contenttype="video/mp4" default="" div="" document.getelementbyid('powerpress_player_'+id)="" document.getelementbyid('powerpress_player_'+id).getelementsbytagname="" document.getelementbyid('powerpress_player_'+id).innerhtml="Html;" embed="" fallback="" false;="" file="" for="" function="" h.264="" height="" html="" if(="" images="document.getElementById('powerpress_player_'+id).getElementsByTagName('img');" images.length="" images[0].src="" insert="" into="" media="" media_url.indexof('.webm')="" mp4="" of="" play="" player="" poster="" powerpress_embed_html5v(id,media_url,width,height,webm_media_url)="" return="" specific="" to="" true;="" type="" url="" var="" video,="" webm="" width="" with="" {="" }=""> -1 )
...
function powerpress_embed_winplayer(div,media_url,width,height)
{
	...
	{
		var Html = '';
		Html += '<object classid="clsid:6BF52A52-394A-11d3-B153-00C04F79FAA6" height="'+ height +'" id="winplayer" standby="loading..." type="application/x-oleobject" width="'+ width +'">\n';
		...
		Html += '	<embed allowchangedisplaysize="1" animationatstart="1" autosize="1" autostart="true" height="'+ height +'" scale="aspect" showcontrols="1" showdisplay="0" showstatusbar="0" src="'+ media_url +'" type="application/x-mplayer2" width="'+ width +'"/>\n';
		Html += '</object>\n';
		document.getElementById(div).innerHTML = Html;
		return false; // stop the default link from proceeding
	}
...
function powerpress_embed_swf(div,media_url,width,height)
{
	...
	{
		var Html = '';
		Html += '<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=6,0,29,0" height="'+ height +'" menu="true" width="'+ width +'">\n';
		Html += '	<param name="movie" value="'+ media_url +'"/>\n';
		Html += '	<param name="quality" value="high"/>\n';
		Html += '	<param name="menu" value="true"/>\n';
		Html += '	<param name="scale" value="noorder"/>\n';
		Html += '	<param name="quality" value="high"/>\n';
		Html += '	<embed height="'+ height +'" menu="true" pluginspage="http://www.macromedia.com/go/getflashplayer" quality="high" src="'+ media_url +'" type="application/x-shockwave-flash" width="'+ width +'"/>';
		Html += '</object>\n';
		document.getElementById(div).innerHTML = Html;
		return false; // stop the default link from proceeding
	}
